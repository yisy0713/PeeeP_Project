...
// 게임 세이브 & 로드
void APPCharacterPlayer::SaveGame()
{
	UPPGameSave* SaveGameInstance = Cast<UPPGameSave>(UGameplayStatics::CreateSaveGameObject(UPPGameSave::StaticClass()));
...
void APPCharacterPlayer::LoadGame()
{
	UPPGameSave* LoadGameInstance = Cast<UPPGameSave>(UGameplayStatics::LoadGameFromSlot(TEXT("PlayerSaveSlot"), 0));

	if (LoadGameInstance)
	{
		SetActorLocation(LoadGameInstance->PlayerLocation);
		SetActorRotation(LoadGameInstance->PlayerRotation);
		//this->Health = LoadGameInstance->PlayerHealth;
...
// 게임 세이브 테스트하려고 만든 함수
void APPCharacterPlayer::TakeDamage(float DamageAmount)
{
	Health -= DamageAmount;
	if (Health < 0.0f)
	{
